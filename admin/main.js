...
function populateUsers() {
    ...
    for(let i=totalUsers-1; i>=0; i--){
        ...
        let status = "";
        if (user.buyer) {
            status = '<button class="btn btn-primary" onclick="switchProfile(\''+user.id+'\', '+false+')\">Change to Seller</button>';
        } else {
            status = '<button class="btn btn-success" onclick="switchProfile(\''+user.id+'\', '+true+')\">Change to Buyer</button>';
        }
        let row = "<tr>"+
